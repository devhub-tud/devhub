				...
				[#if states.hasFinished(commit.getCommit())]
					[#if states.hasSucceeded(commit.getCommit())]
						<td class="commit succeeded">
							...
							<a href="${path}/commits/${commit.getCommit()}/diff" class="btn btn-default btn-sm pull-right" style="margin: 5px;">
								<span class="glyphicon glyphicon-indent-left"></span> View diff
							</a>
