...
public class ProjectPullTest extends WebTest {
	...
	public void createRepository() throws URISyntaxException, GitAPIException, IOException {
		Repository repository = repositoriesManager.getRepository(new URI(groupRepository.getRepositoryName() + ".git/"));

		Git git = Git.cloneRepository()
			.setBare(false)
			.setDirectory(temporaryFolder.getRoot())
			.setRemote(repository.getPath().toString())
			.call();

		git.checkout().setName("my-super-branch").call();
		...
		git.push().setRemote("origin/my-super-branch").call();
	}
